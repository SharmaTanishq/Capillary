...
export async function sendOrderDetails({
  ...
  orderDetails: Order;
  orderItems: CommerceRuntimeOrderItem[];
}): Promise<CapillaryTransactionResponse> {
  if (!orderDetails) {
    return {
      success: false,
  ...
  const mappingResult =
    await KiboToCapillaryOrderMapper.mapOrderToCapillaryFormat(
      orderDetails,
      orderItems
    );
  ...
  const storeId =
    orderDetails?.items?.[0]?.fulfillmentMethod === "Pickup"
      ? orderDetails?.items?.[0].fulfillmentLocationCode
      : "4000";
